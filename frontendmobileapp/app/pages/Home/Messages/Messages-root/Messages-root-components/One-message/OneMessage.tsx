...
const OneMessage = ({ message }: Props) => {
  return (
    <View style={[styles.container, message?.unread && styles.activeBackgroundColor]}>
      {message?.unread && <View style={styles.activeIndicator}></View>}

      <Image
        source={getImageSource(message?.profile_pic, "profile")}
      ...
      <View style={styles.messageContainer}>
        ...
        <Text
          numberOfLines={2}
          ellipsizeMode="tail"
          style={!message?.unread ? styles.messageOpened : styles.messageUnopened}
        >
      ...
      <View style={styles.timeContainer}>
        <Text style={styles.time}>
          {dayjs(message?.last_message_time).format("MMM D, YYYY h:mm A")}
        </Text>
      </View>
    </View>
  );
