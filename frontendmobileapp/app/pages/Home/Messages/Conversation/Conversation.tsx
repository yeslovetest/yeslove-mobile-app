...
const Conversation = () => {
  ...
  const otherUserId = useAppSelector((state) => state.navigation.tabStack.at(-1)?.data?.userId);
  const otherUserProfilePic = useAppSelector(
    (state) => state.navigation.tabStack.at(-1)?.data?.profile_pic,
  );
  const otherUserName = useAppSelector(
    (state) =>
  ...
  useEffect(() => {
    ...
    if (sendMessageStatus === "failed" && previousStatus !== "failed") {
      ...
      Alert.alert(
        "Send failed",
        sendMessageError || "Unable to send your message right now. Please try again.",
      );
      dispatch(resetSendChatMessageStatus());
    }
        ...
        {hasSelectedMedia && (
          ...
          >
            <MediaFilePreview
              file={selectedFiles ?? []}
              editable={true}
              deleteMedia={deleteSelectedFile}
            />
          </Animated.View>
        )}
