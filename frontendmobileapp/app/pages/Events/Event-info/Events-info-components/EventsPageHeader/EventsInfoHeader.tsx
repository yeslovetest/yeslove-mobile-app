...
const EventsInfoHeader = () => {
  ...
  const event: EventsModelResponse = useAppSelector(
    (state) => state.navigation.tabStack.at(-1)?.data,
  ) as EventsModelResponse;
  ...
  const eventImageSource =
    hasRemoteImage && !useFallbackImage ? { uri: imageUrl } : defaultEventImg;
      ...
      >
        {isFuture && (
          <Animated.View style={[styles.favouriteContainer, !isAttending && addEventAnimatedStyle]}>
            <Pressable
              onPress={() => toggleAttending(isAttending)}
              style={styles.favouritePressable}
            >
              <Text style={[styles.addToEventText, isAttending && styles.attendingText]}>
                {isAttending ? "Attending" : "Add Event"}
              </Text>
            </Pressable>
          </Animated.View>
        )}
        <View style={styles.overlayInd}>
          ...
          <Text style={styles.eventNameInd} numberOfLines={2}>
            {event.name || "Event"}
          </Text>
          <Text style={styles.eventLocationInd} numberOfLines={1}>
